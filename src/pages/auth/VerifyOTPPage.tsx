...
const VerifyOTPPage: React.FC = () => {
  ...
  const [purpose, setPurpose] = useState<'registration' | 'login'>('registration');
  const [redirect, setRedirect] = useState<string | null>(null);
  const inputRefs = useRef<(HTMLInputElement | null)[]>([]);

  useEffect(() => {
    // Get email and purpose from location state
    const state = location.state as { email?: string; purpose?: 'registration' | 'login'; redirect?: string };
    if (state?.email) {
      setEmail(state.email);
      setPurpose(state.purpose || 'registration');
      if (state.redirect) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (purpose === 'registration') {
        response = await userApi.verifyOTP(email, otpString);
      } else {
        response = await userApi.verifyLoginOTP(email, otpString);
      }
      ...
      if (response.data) {
        ...
        if (purpose === 'registration') {
          ...
          // For login, redirect based on redirect param or role
          if (redirect) {
  ...
  const handleResendOTP = async () => {
    ...
    try {
      const response = await userApi.resendOTP(email, purpose);
      if (response.error) {
        setError(response.error);
      } else {
        setError('');
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gray-50 py-8 sm:py-12 px-4 sm:px-6 lg:px-8 bottom-nav-safe">
      <div className="max-w-md w-full space-y-6 sm:space-y-8">
        ...
        <div className="text-center">
          <Link 
            to={purpose === 'registration' ? "/auth/register" : "/auth/login"}
